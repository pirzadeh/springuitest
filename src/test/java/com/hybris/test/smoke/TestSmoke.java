...
public class TestSmoke extends SeleniumTestCase { 
	...
	public void testHomePageSearch() {
		...
		PowerSearchResultsPage searchResultsPage = homePage.searchFor(SEARCH_TERM);
		// Then I should navigate to search results page
		Assert.assertTrue(webDriver.getCurrentUrl().contains("/search?text="));

		PowerProductDetailsPage productDetailsPage = searchResultsPage.selectProduct(
				"Cordless-drill-driver-2006");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id='addToCartButton']")));
	...
	@Test
	@Category(LoginFeature.class)
	public void testHomePageLogin() {

		System.out.println("3");

		PowerHome homePage = site.gotoPowerHome();
		PowerLoginPage loginPage = homePage.login();
